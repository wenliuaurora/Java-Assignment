...
class Record{
    ...
    public void SetData(String[] inputs){
        for(int i=0;i<inputs.length;i++){
            data.add(inputs[i]);
        }
    }
    ...
    private static void test(){
        ...
        testRecord1.SetData(new String[]{"male"});
        testRecord1.SetData(new String[]{"female","Bristol"});
        assert(testRecord1.Getdata(4)=="male");
        assert(testRecord2.Getdata(4)=="female");
        assert(testRecord2.Getdata(5)=="Bristol");
        
        assert(testRecord1.UpdateData("female", 4)==true);
        assert(testRecord2.UpdateData("male", 4)==true);
        assert(testRecord1.Getdata(4)=="female");
        assert(testRecord2.Getdata(4)=="male");

        

        assert(testRecord1.GetSize()==5);
        assert(testRecord1.GetSize()==6);
